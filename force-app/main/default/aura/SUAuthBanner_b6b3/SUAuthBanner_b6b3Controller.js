({
    ...
    searchButtonPress: function (component, event, helper) {
        ...
        var a = component.get('c.submitForm');
        $A.enqueueAction(a);
    },
    fillSearchBoxOnSuggest: function (component, event, helper) {
        ...
        if (window.location.href.split('#')[0].split('?')[0] == component.get('v.currentCommunityURL')) {
    ...
    fillSearchbox: function (component, event, helper) {
        ...
        var suggest = event && event.target ? event.target.getAttribute("data-suggest") : null;
        if (component.get("v.isEnableTitleRedirect") && !suggest) {
            var target;
            if (event && event.target && event.target.closest('span') || (component.get('v.linkToOpen') && component.get('v.dataObj'))) {
                target = event && event.target ? event.target.closest('span') : null;
                var dataObj = component.get('v.dataObj') || {};
                var data_Id = dataObj.data_Id || target.getAttribute("data-id") || "";
                var data_type = dataObj.data_type || target.getAttribute("data-type") || "";
                var data_index = dataObj.data_index || target.getAttribute("data-index") || "";
                var data_rank = dataObj.data_rank || target.getAttribute("data-rank") || "";
                var data_url = dataObj.data_url || target.getAttribute("data-url") || "";
                var data_sub = dataObj.data_sub || target.getAttribute("data-subject") || "";
                var result_count = dataObj.result_count || target.getAttribute("data-Count") || 0;
                var data_trackAnalytics = dataObj.data_trackAnalytics || target.getAttribute("data-trackAnalytics") || [];
                if (data_rank != '') {
            ...
            if(component.get("v.redirectThroughUrl")) {
                ...
                if (sourceType !== "" && sourceValue !== "") {
                    var filterData = { "sourceType": sourceType, "valueFilter": sourceValue };
                    helper.createAggregationFilter(component, helper, filterData, 1);
                }
                helper.redirectFunc(component, valueToBeFilled, helper);
            }
            else
    ...
    submitForm: function (component, event, helper) {
        ...
        if (component.get("v.isEnableTitleRedirect") && component.get('v.linkToOpen')) {
            var a = component.get('c.fillSearchbox');
            $A.enqueueAction(a);
            /*var url = event.currentTarget.dataset.url;
            var urlEvent = $A.get("e.force:navigateToURL");
            urlEvent.setParams({
                "isredirect": true,
                "url": component.get('v.linkToOpen')
            });
            urlEvent.fire();*/
        } else {
            ...
            if (SearchQuery != null && SearchQuery != '' && SearchQuery != undefined && SearchQuery.length > 1) {
                ...
                if(component.get("v.redirectThroughUrl")) {
                    ...
                    if (window.location.href.split('#')[0].split('?')[0] == component.get('v.currentCommunityURL')) {
                        ...
                        var SearchQuery = component.get("v.searchString");
                        SearchQuery = SearchQuery.trim();
                        component.set("v.firstLoad", 1);
                        helper.redirectFunc(component, SearchQuery, helper);
                    }
                }
                else {
                    component.set("v.firstLoad", 1);
                    window.location.hash = encodeURIComponent("searchString=" + encodeURIComponent(component.get("v.searchString")));
                    helper.searchButtonPressFunc(component, event, helper);
                }
            }
            else {
    ...
    smartAutocompleteSelection: function(component, event, helper){
        event.stopPropagation();
        component.set("v.smartFacetClick", true);
        let smartIndex = event && event.target ? event.target.getAttribute("data-Smart") : null;
        let filterIndex = event && event.target ? event.target.getAttribute("data-Filter") : null;
        let smart;
        let filter;
        let smartAggregations = component.get("v.smartAggregations");
        
        smartIndex = +smartIndex;
        filterIndex = +filterIndex;
        
        smart = smartAggregations[smartIndex];
        filter = smartAggregations[smartIndex].values[filterIndex];

        var clickedOrder = smartAggregations.findIndex(function(f){
        	return f.order === smart.order});

        var index = -1;
        var aggrValue = component.get("v.aggregation");
        if(aggrValue.length){
            aggrValue = JSON.parse(aggrValue);
        }
        let aggregations = helper.getURLParameter('selectedType') ? JSON.parse(helper.getURLParameter('selectedType')) : aggrValue; 

        let some = aggregations.some(function (facet, i) {
          if (facet.type == smartAggregations[clickedOrder].key) {
            index = i;
            return true;
          }
        });
        ...
        if (smart.key != 'post_time') {
          if (filter.selected) {
            ...
            document.getElementById(filter.Contentname + '_smartFacet').classList.remove('su__product-active');  
            if (aggregations[index] && aggregations[index].type == filter.parent && aggregations[index].filter.length > 1) {
                let moreFilters = aggregations[index].filter.filter(function(remove){
                    return remove != filter.Contentname
               });
                aggregations[index].filter = moreFilters;
            } else {
                let singlefilters = aggregations.filter(function(remove){
                    return remove.type != filter.parent
                });
              aggregations = singlefilters;
            }
          } else {
            filter.selected = true;
            document.getElementById(filter.Contentname + '_smartFacet').classList.add('su__product-active');       
            if (aggregations[index] && aggregations[index].type == filter.parent && aggregations[index].filter.indexOf(filter.Contentname) != -1) {
        ...
        } else if ((aggregations[index].type == 'post_time' || aggregations[index].type == 'CreatedDate') && !filter.selected) {
          filter.selected = true;
            smart.values.forEach(function(cdate){
        ...
        component.set("v.aggregation", JSON.stringify(aggregations));
    ...
    clearSearchInputbox:function(component, event, helper) {
