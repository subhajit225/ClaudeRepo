import getCommunityCustomSettings from '@salesforce/apex/su_vf_console.SUAuthentication.getCommunityCustomSettings';
...
const fetchCommunitySettings = () => {
    return getCommunityCustomSettings({ "authParams": { "permission": '' } }).then(result => {
        if (result && result.isCustomSettingFilled) {
            loadScriptStyleFunc(result.cdnSupportEndPoint+'/'+result.uid);
            ...
            var setEmail = setInterval(() => {
                if(scriptLoaded) {
                    clearInterval(setEmail);
                    try {
                        window.GzAnalytics && window.GzAnalytics.setUser(currentUserEmail);
                    } catch (error) {
            ...
            result.customSettingErrorMessage = 'Please configure your SearchUnify and try again.';
        }
        setCommunitySettings(result);
        return result;
    })
    .catch(error => {
        console.log('[error]', error);
...
const setCommunitySettings = (result) => {
    communitySettings = result;
}

const getCommunitySettings = () => {
    return new Promise((resolve) => {
        var check = setInterval(() => {
            if(Object.keys(communitySettings).length) {
                clearInterval(check);
                resolve(communitySettings);
            }
        }, 10);
    });
}
...
const loadScriptStyleFunc = (s3EndPoint) => {
    if(!scriptLoaded && !styleLoaded) {
        ...
        .catch((error) => {
            styleLoaded = 'Not valid CDN Endpoint';
    ...
    return new Promise((resolve) => {
        var check = setInterval(() => {
            if(scriptLoaded && styleLoaded) {
                ...
                fetchLabels();
                resolve(true);
            } else if(Object.keys(communitySettings).length) {
                if(!communitySettings.isCustomSettingFilled) {
                    clearInterval(check);
                    resolve(true); 
                } else if(scriptLoaded === 'Not valid CDN Endpoint' || styleLoaded === 'Not valid CDN Endpoint') {
                communitySettings.isCustomSettingFilled = false;
                communitySettings.customSettingErrorMessage = "please verify your CDN endpoint and re-configure your searchunify";
                clearInterval(check);
                resolve(true); 
            } 
            } 
...
const getURLParameter = (param) => {
    ...
    if(m.location.href.includes('#')){
        let queryParams;
        queryParams = m.location.href.split('#')[1];
