...
import search from '@salesforce/apex/SU_Knowbler.KCSPublishController.search';

export default class KnowblerEmailInput extends LightningElement {
  @track items = [];
  ...
  set selectedValues(value) {
    ...
    const selectedValuesEvent = new CustomEvent('selection', {
  ...
  handleBlur() {
    ...
    this.blurTimeout = setTimeout(() => {
      this.boxClass =
        'slds-combobox slds-dropdown-trigger slds-dropdown-trigger_click slds-has-focus';
      const { value } = this.template.querySelector('input.input');
  ...
  onSelect(event) {
    ...
    const selectedValue = this.items.find((record) => record.Id === selectedId);
