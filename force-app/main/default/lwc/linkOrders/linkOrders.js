...
import updateOrderLinks from '@salesforce/apex/LinkOrderController.updateHWAndRelatedOrder'; 
import { ShowToastEvent } from 'lightning/platformShowToastEvent';
import { CloseActionScreenEvent } from 'lightning/actions';
import { NavigationMixin } from 'lightning/navigation';
export default class LinkOrders extends LightningElement {
    @track error ;
    @track hideSpinner = false;
    @api isInsideVF = false;
    loadData() {
		updateOrderLinks({ orderId: this.recordId })
		.then(result => {
            ...
            this.hideSpinner= true;
            if(this.isInsideVF){
                alert('Orders are linked successfully');
                //window.history.back();
                window.location.href = '/'+this.recordId;
            }else{
                this.showToast('Orders are linked successfully', 'success','Success');
