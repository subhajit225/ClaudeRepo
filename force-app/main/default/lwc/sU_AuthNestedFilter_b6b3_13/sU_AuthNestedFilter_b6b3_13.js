...
export default class SU_AuthNestedFilter extends LightningElement {
    ...
    setFilters(setArray) {
        ...
        for (var i = 0; i < this.aggregationsData.length; i++) {
            ...
            let stringifiedPathParent = JSON.stringify(this.aggregationsData[i].path)
            ...
            for (var j = 0; j < this.aggregationsData[i].childArray && this.aggregationsData[i].childArray.length; j++) {
                let stringifiedPath = JSON.stringify(this.aggregationsData[i].childArray[j].path)
